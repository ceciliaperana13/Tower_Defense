...
private:
    ...
    // Index 0 = basic (always visible), 1-4 = upgrades (visible only when tower selected)
    ...
    // Highlight ring drawn around the selected tower on the map
    sf::RectangleShape m_towerHighlight;

    // Placement overlay (rebuilt once per buy-selection)
    std::vector<sf::RectangleShape> m_placementOverlay;
    bool                            m_overlayDirty { true };

    // Hover cell (2x2 block under cursor while placing)
